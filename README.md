...
A self-hosted bodyweight and calorie tracking web app. Built with Vue 3, PocketBase, and Tailwind CSS.
...
curl -o docker-compose.yml https://raw.githubusercontent.com/Giton22/bodyweight-tracker/main/docker-compose.yml
...
The container is tagged with [semantic versions](https://github.com/Giton22/bodyweight-tracker/releases).
To pin to a specific version, change `latest` to a version tag in `docker-compose.yml`:

```yaml
image: ghcr.io/giton22/bodyweight-tracker:1.0.0
```
...
```sh
docker run --rm \
  -v bodyweight-tracker_pb_data:/data \
  -v $(pwd):/backup \
